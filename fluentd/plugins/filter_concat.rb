  ...
  class ConcatFilter < Filter
    ...
    def filter_stream(tag, es)
      ...
      es.each do |time, record|
        ...
        if record.key?("_timeout_contact") and record["_timeout_contact"]=="true"
          record.delete("_timeout_contact")
          new_es.add(time, record)
          next
        end
        if @mode == :partial
    ...
    def flush_timeout_buffer
      ...
      @timeout_map.each do |stream_identity, previous_timestamp|
        ...
        @timeout_map_mutex.synchronize do
          ...
          flushed_record["_timeout_contact"]="true"
          new_es.add(@use_first_timestamp ? time : now,flushed_record)
          @timeout_map.delete(stream_identity)  
        end
    
        tag = stream_identity.split(":").first
        handle_timeout_error(tag, new_es, new_es)
    ...
    def flush_remaining_buffer
      @buffer.each do |stream_identity, elements|
        ...
        handle_timeout_error(tag, time, new_es)
      end
      @buffer.clear
    end

    def handle_timeout_error(tag, time, new_es)
      if @timeout_label
        event_router = event_emitter_router(@timeout_label)
        event_router.emit(tag, time, record)
      else
        router.emit_stream(tag,new_es)
      end
    end
  end
end
